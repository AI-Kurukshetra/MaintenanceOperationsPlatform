...
import { workOrderUpdateSchema, type WorkOrderUpdateInput } from "@/lib/validations/work-order";
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" role="combobox" className="w-full justify-between font-normal">
          {selected ? <span className="truncate">{renderLabel(selected)}</span> : <span className="text-muted-foreground">{placeholder}</span>}
...
export default function EditWorkOrderPage() {
  ...
  const form = useForm<WorkOrderUpdateInput>({
  ...
  useEffect(() => {
    ...
    async function loadWorkOrder() {
      ...
      try {
        ...
        if (!mounted || !workOrder) {
          return;
        }

        if (workOrder.status === "completed" || workOrder.status === "cancelled") {
          toast.info("Completed or cancelled work orders cannot be edited.");
          router.push(`/work-orders/${params.id}`);
          return;
        }

        setWoNumber(workOrder.wo_number);
        reset({
          ...
          due_date: workOrder.due_date
            ? new Date(workOrder.due_date).toISOString().slice(0, 16)
            : "",
        ...
        const rows =
          workOrder.work_order_parts?.map((item) => ({
        ...
        const partRecord: Record<string, PartOption> = {};
        for (const row of workOrder.work_order_parts ?? []) {
          if (row.parts && row.part_id) {
            partRecord[row.part_id] = {
              id: row.parts.id,
              name: row.parts.name,
              part_number: row.parts.part_number,
              quantity_on_hand: row.parts.quantity_on_hand,
            };
          }
        }
        setPartMeta(partRecord);
      } finally {
        if (mounted) {
          setIsLoading(false);
        }
  ...
  }, [params.id, replace, reset, router]);
  ...
  const onSubmit = async (input: WorkOrderUpdateInput) => {
    if (!canEdit) {
      toast.error("You do not have permission to edit work orders.");
      return;
    }

    setIsSubmitting(true);
    try {
      const payload: WorkOrderUpdateInput = {
        ...input,
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <PageHeader title="Edit Work Order" description={`WO Number: ${woNumber}`} />
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <Card>
          ...
          <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <Select
                value={watch("type")}
                onValueChange={(value) => setValue("type", value as WorkOrderUpdateInput["type"])}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setValue("priority", value as WorkOrderUpdateInput["priority"])
                }
              >
                <SelectTrigger>
                  <SelectValue />
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {fields.length === 0 ? <p className="text-sm text-muted-foreground">No parts added.</p> : null}
